 ...
 */
Ext.define('TYPO3.Vidi.Components.Tree.Editor', {
	extend: 'Ext.grid.plugin.CellEditing',
	alias: 'plugin.vidi-treeeditor',
	...
	listeners: {
		beforeedit: {
			fn: function (editEvent) {
				...
				if (editEvent.record == tree.currentSelectedNode) {
		...
		validateedit: {
			fn: function (treeEditor, editEvent) {
				...
				if (this.newValue === '' || this.newValue === editEvent.originalValue) {
					...
					editorField.setValue(Ext.util.Format.htmlEncode(this.newValue));
				}
	...
	updateNodeText: function (record, editableText, updatedText, dataIndex, tree) {
		//record.set(updatedText);
		record.set('text', editableText);
		record.commit();
